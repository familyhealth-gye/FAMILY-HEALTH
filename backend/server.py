...
async def create_pediatric_history(
    ...
    # VALIDACIÓN CRÍTICA: Verificar que el doctor tenga permisos para Pediatría
    if user.get('role') == 'Doctor':
        appointment_especialidad = appointment.get('especialidad', '')
        
        if appointment_especialidad != 'Pediatría':
            raise HTTPException(
                status_code=403, 
                detail=f"No tiene permisos para crear historias de {appointment_especialidad}. Este formulario es para Pediatría."
            )
